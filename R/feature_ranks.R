...
feature_ranks <- function(model, meth = "gini")
  {
  # standard check here
  ...
  if(meth == "perm"){
    if(! "MeanDecreaseAccuracy" %in% colnames(imp_df)){
      ...
      imp_vals <- data.frame(variable = rownames(imp_df), importance = imp_df[,"MeanDecreaseAccuracy"], row.names = NULL)
    }
  }

  if(meth == "gini"){
    imp_vals <- data.frame(variable = rownames(imp_df), importance = imp_df[,"MeanDecreaseGini"], row.names = NULL)
  }

  rank_ord <- rank(-abs(imp_vals$importance), na.last = TRUE, ties.method = "random")
