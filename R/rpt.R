...
#'
#' Calculates the Robustness Performance Trade-off of a model
#'
...
rpt <- function(stability, performance, beta = 1)
  {
  if(length(stability) != 1){stop("...stability must be a numeric vale NOT a vector", call. = FALSE)}
  if(length(performance) != 1){stop("...performance must be a numeric vale NOT a vector", call. = FALSE)}
  if(stability < 0 | stability > 1){stop("..stability  must be between 0 and 1", call. = FALSE)}
  if(performance < 0 | performance > 1){stop("...performance must be between 0 and 1", call. = FALSE)}
